...
def run_phase3():
    ...
    # Let's take the first 2000 examples for the train set
    dataset = load_dataset("pminervini/HaluEval", "qa", split="data[:2000]")
    df = pd.DataFrame(dataset)
    
    out_dir = "phase3_results"
    os.makedirs(out_dir, exist_ok=True)
    out_csv = f"{out_dir}/train_features.csv"
    
    # We will stream results to CSV to avoid losing data if it crashes
    with open(out_csv, 'w') as f:
        # Write header placeholder, will write real header on first row
