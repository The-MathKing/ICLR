...
| S13 | **Prose that the regeneration invalidated, rewritten.** Four single-value macros became ranges once there were two sink-less models, leaving sentences asserting things they no longer said: the SmolLM contrast paragraph, the "near zero for SmolLM" claim (the correlation with the sink score is in fact *negative*, -0.16 to -0.09, in both), the "(... in SmolLM)" aside, the exceptions list, Figure 2's caption and the section 4 model list. All now macro-driven. The abstract and Limitations use `\ModelSizeRange` instead of a hardcoded 1.1B--3.8B. The Mistral appendix is rewritten from generated numbers and its "we have not verified delta_0 directly" hedge is gone, because it is now verified. Limitations no longer claims the labels are unaudited without qualification. Section 5.4 reports the agreement with the authors' released code. | `paper/paper.tex`, `paper/sink_results.tex` |
| S14 | **The paper compiles, and the main text fits.** MiKTeX turned out to be installed already, just not on PATH. 36 pages, **0 undefined references, 0 overfull boxes**. Adding five settings had widened Tables 1--4 and pushed the main text onto page 10 -- confirmed pre-existing by compiling the previous commit, not caused by the prose edits. Table 3 (key paired comparisons) moved to Appendix "Full results", which already carried the full versions of both that table and the AUC table, so nothing is lost and `tab:tests` still resolves. Limitations now ends on page 9 and the reproducibility statement starts on page 10. Tables 1 (16 rows) and 3 (15 rows) rendered and inspected: legible. | `paper/paper.pdf` |

**Not done in this session, and why.**
1. **Llama-3.1-8B**: dropped by decision on 2026-09-17. A token was added, but Meta's licence is granted per account and manual, and this account is not on the authorized list, so the weights 403 anyway. Downloadable community mirrors were declined: their bit-identity to Meta's weights cannot be verified while the official repo is blocked, and a third 7--8B model is not the evidence that moves the score (TOHA's own benchmarks are). Everything needed to run it later is in place.
