# Fills the one empty cell in the model x benchmark grid: Qwen2.5-1.5B was
# previously evaluated on HaluEval QA only, which forced the paper to caveat
# that its 1.5B-vs-3B scale comparison is cross-benchmark. Generated from
# phase10_qwen3b_eval.py with the model id and output path changed; the
# extraction protocol (chat template + system turn) is identical, so the two
...
def run_extraction():
    ...
    out_csv = "phase10_results/qwen1_5b_truthfulqa_4stat.csv"
